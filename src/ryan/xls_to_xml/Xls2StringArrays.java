...
public class Xls2StringArrays {
    ...
    public static void main(String[] args) {
        ...
        for (File excelFile : files) {
            ...
            LinkedHashMap<String, String> sortedMaps = getSortedMaps(excelMaps);
            ...
            for (String each : keys) {
                if (each.lastIndexOf(fileName) != -1 && each.lastIndexOf(fileName) != 4) {
                    //找到了对应和xls文件语言对应的values目录
                    String valuesDir = stringsFilePathList.get(each);
                    String arraysFile = valuesDir + File.separator + "arrays.xml";
                    try {
                        WriteToXML.appenArrays2XML(arraysFile, "titles", sortedMaps);
                    } catch (IOException e) {
                        e.printStackTrace();
    ...
    private static LinkedHashMap<String, String> getSortedMaps(Map<String, String> excelMaps) {
        LinkedHashMap<String, String> sortedMaps = new LinkedHashMap<>();
        for (int i = 0; i < LIST_NEED_TO_READ.length; i++) {
            String s = LIST_NEED_TO_READ[i];
            sortedMaps.put(s, excelMaps.get(s));
        }
        return sortedMaps;
    }
        ...
        for (File f : file1) {
            if (f.isDirectory()) {
                ...
                if (f.getName().trim().equals("strings.xml")) {
                    stringsFilePathList.put(f.getParentFile().getName(), f.getParentFile().getAbsolutePath());
                }
            }
